...
main = do togglevga
          ...
          let myStatusBar = "dzen2 -x 0 -w '" ++ screenWidth ++ "' -h '28' -ta l -xs 1 -fg '" ++ foreground ++ "' -bg '" ++ background ++ "' -fn '" ++ myFont ++ "'"
          let myConkyBar = "conky -c ~/.xmonad/conky_dzen | dzen2 -ta r -x '" ++ screenWidth ++ "' -w '" ++ screenWidth ++ "' -h '28' -p $OPTS -xs 1 -fg '" ++ foreground ++ "' -bg '" ++ background ++ "' -fn '" ++ myFont ++ "'"
	  dzproc <- spawnPipe $ myStatusBar
          dzstatus <- spawnPipe $ myConkyBar
          -- dock <- spawnPipe "GTK2_RC_FILES=/home/martinr/.xmonad/gtkdocky /usr/bin/docky"
...
myLayoutHook = smartBorders $ avoidStruts $
               ...
               Full
               where
                 verticalTile = spacing 8 $ Tall 1 (3/100) (1/2)
                 horizTile = Mirror $ spacing 8 $ Tall 1 (3/100) (1/2)
...
myLogHook dzproc = dynamicLogWithPP $ dzenPP {
    ...
    , ppTitle =  wrap "^ca(1,/home/martinr/.xmonad/scripts/popterm.sh)" "^ca()" . pad  . shorten 50
    , ppLayout = dzenColor color4 background . (\x -> case x of
                                             "SmartSpacing 5 Tall" -> "    ^i(/home/martinr/.xmonad/icons/tiling.xbm)   "
                                             "Mirror SmartSpacing 5 Tall" -> "    ^i(/home/martinr/.xmonad/icons/mirrortall.xbm)   "
                                             "Full" -> "    w^i(/home/martinr/.xmonad/icons/floating.xbm)   "
                                             _ -> "   New Layout   "
                                             )
...
myWorkspaces = [music, web, code, devel, media, mail, misc]
               where music = "   ^i(/home/martinr/.xmonad/icons/Music.xbm)   "
                     web   = "   ^i(/home/martinr/.xmonad/icons/www.xbm)   "
                     code  = "   ^i(/home/martinr/.xmonad/icons/code.xbm)   "
                     devel = "   ^i(/home/martinr/.xmonad/icons/Devel.xbm)   "
                     media = "   ^i(/home/martinr/.xmonad/icons/media.xbm)   "
                     mail  = "   ^i(/home/martinr/.xmonad/icons/mail.xbm)   "
                     misc  = "   ^i(/home/martinr/.xmonad/icons/pacman.xbm)   "
